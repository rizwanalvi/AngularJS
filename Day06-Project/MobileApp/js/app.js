...
mApp.controller('PhoneDetailController',function($scope, $routeParams) { 
    // console.log(phone.filter(x=>x.id=='motorola-xoom-with-wi-fi'));
    $scope.msg =phone.filter(x=>x.id==$routeParams.phoneId);
    
});
  mApp.controller('MobileController',function($scope){
    $scope.curPage = 1,
    $scope.itemsPerPage = 3,
    $scope.maxSize = 5;
    $scope.phones=filteredItems= phone;
    $scope.numOfPages = function () {
        return Math.ceil(phones.length / $scope.itemsPerPage);
          
      };
     $scope.$watch('curPage + numPerPage', function() {
        var begin = (($scope.curPage - 1) * $scope.itemsPerPage),
        end = begin + $scope.itemsPerPage;
        $scope.filteredItems = phones.slice(begin, end);
      }); 
});